...
RegisterNetEvent('wolz-towing:openTowingMenu')
AddEventHandler('wolz-towing:openTowingMenu', function()
    OpenTowingMenu()
end)
...
function DetachRope()
    if entity1 and entity2 then
        CloseTowingMenu()
        TriggerServerEvent('wolz-towing:stopTow')
...
function AttemptAttachRope()
    if entity1 and entity2 then
        ...
        TriggerServerEvent('wolz-towing:tow', veh1, veh2)
        CloseTowingMenu()
...
RegisterNetEvent('wolz-towing:removeRope')
AddEventHandler('wolz-towing:removeRope', function(id, veh1_, veh2_)
    for k, rope in pairs(ropes) do
        if rope.id == id then
...
RegisterNetEvent('wolz-towing:makeRope')
AddEventHandler('wolz-towing:makeRope', function(veh1_, veh2_, id, owner)
    for k, rope in pairs(ropes) do
        if rope.id == id then
            DeleteRope(rope.rope)
            ropes[k] = nil
